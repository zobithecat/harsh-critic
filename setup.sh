#!/bin/bash
# 윈도우 줄바꿈 문자(\r)가 섞여 있어도 안전하도록 경로 처리
mkdir -p .claude/rules 2>/dev/null

RULES_PATH=".claude/rules/harsh-critic.md"
FEEDBACK_PATH=".claude/feedback.jsonl"

# printf를 사용해 중첩 Heredoc 이슈 방지
printf "# 🎭 Harsh Critic Protocol (v1.1)
